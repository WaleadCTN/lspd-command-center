...
// Firebase Auth + Firestore + Officer Management + FTO Evaluations
...
function isChief(){
  return window.LSPD.profile?.role === "Chief";
}

function isFTO(){
  return ["FTO","Sergeant","Lieutenant","Captain","Deputy Chief","Assistant Chief","Chief"]
    .includes(window.LSPD.profile?.role);
}

function isCommand(){
  return ["Sergeant","Lieutenant","Captain","Deputy Chief","Assistant Chief","Chief"]
    .includes(window.LSPD.profile?.role);
}
...
function showApp(){
  ...
  if($("currentUser")){
    $("currentUser").textContent = window.LSPD.user?.email || "Connecté";
  }

  if($("userPill")){
    $("userPill").textContent =
      `${window.LSPD.profile?.grade || "Officer"} • ${window.LSPD.profile?.role || "Officer"}`;
  }
}
...
function applyRoleVisibility(){
  const officerBtn = document.querySelector('#nav button[data-page="officers"]');
  const historyBtn = document.querySelector('#nav button[data-page="history"]');

  officerBtn?.classList.toggle("hidden", !isCommand());
  historyBtn?.classList.toggle("hidden", !isCommand());
}
...
async function dashboard(){
  ...
  try{
    const q = query(
      ...
      where("officerId","==",window.LSPD.user.uid)
    );
    const snap = await getDocs(q);
    evals = snap.docs.map(d => d.data());
  }catch(err){
    console.warn("Dashboard evaluations :", err);
  }

  const validatedCodes = [...new Set(
    evals
      .filter(e => e.result === "Validé")
      .map(e => e.moduleCode)
  )];

  const validated = validatedCodes.length;
  const pct = Math.round((validated / modules.length) * 100);

  $("content").innerHTML = `
    <div class="grid stats-grid">
      <div class="card">
        <div class="muted">Identité</div>
        <div class="stat">${esc(p?.name)}</div>
        <div class="muted">${esc(p?.badge)}</div>
      </div>

      <div class="card">
        <div class="muted">Grade</div>
        <div class="stat">${esc(p?.grade)}</div>
      ...
      <div class="card">
        ...
        <div class="muted">${validated}/${modules.length} modules validés</div>
      </div>

      <div class="card">
        <div class="muted">Statut</div>
        <div class="stat">${esc(p?.status)}</div>
        <div class="muted">profil Firebase</div>
      </div>
    </div>
    ...
    <div class="grid2">
      <div class="card">
        ...
        ${modules.slice(0,8).map(m => {
          const ok = validatedCodes.includes(m[0]);
          return `
            <div class="row">
              <span>${m[0]} — ${m[1]}</span>
              <span class="tag ${ok ? "green" : ""}">${ok ? "Validé" : "À faire"}</span>
            </div>
          `;
        }).join("")}
      </div>
      ...
      <div class="card">
        <h3>Session</h3>
        <p><b>${esc(p?.name)}</b></p>
        <p class="muted">${esc(window.LSPD.user?.email)}</p>
        <p class="muted">${isFTO() ? "Accès FTO / Command actif." : "Accès Officer."}</p>
      </div>
    </div>
  `;
}
...
function manual(){
  const standards = [
    ...
    "Expliquer le pourquoi de chaque procédure",
    "Traiter immédiatement les erreurs critiques",
    "Feedback factuel et précis",
    "Chaque validation doit être traçable",
    "Même standard pour nouveaux et anciens"
  ];

  $("content").innerHTML = `
    <div class="card">
      <h2>Manuel FTO LSPD</h2>
      <p class="muted">
        Briefing → démonstration → pratique → observation → feedback → validation → traçabilité.
      </p>
    </div>

    <div class="section-title">Standards FTO</div>
    <div class="grid2">
      ${standards.map(x => `<div class="card"><b>${x}</b></div>`).join("")}
    </div>
  `;
}
...
async function modulesPage(){
  ...
  $("content").innerHTML = `
    <div class="grid module-grid">
      ${modules.map(m => {
        const moduleEvals = evals
          .filter(e => e.moduleCode === m[0])
          .sort((a,b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));

        const latest = moduleEvals[0];

        return `
          <div class="card module" data-module="${m[0]}">
            <span class="number">${m[0]} • ${m[3]}</span>
            <h3>${m[1]}</h3>
            <p class="muted">${m[2]}</p>
            <span class="tag ${
              latest?.result === "Validé" ? "green" :
              latest?.result === "Échec" ? "red" :
              latest ? "orange" : ""
            }">${latest?.result || "À faire"}</span>
          </div>
        `;
      }).join("")}
    </div>
...
async function evaluations(){
  $("content").innerHTML = `
    <div class="card"><p class="muted">Chargement des évaluations...</p></div>
  `;
  ...
  try{
    ...
    if(isCommand()){
      ...
      const q = query(
        collection(db,"evaluations"),
        where("ftoId","==",window.LSPD.user.uid)
      );
            ...
            ${
              ...
              ? data.map(e => `
                  <tr>
                    ...
                    <td>${esc(e.officerName)}</td>
                    <td>${esc(e.ftoName)}</td>
                    <td>${esc(e.moduleCode)} — ${esc(e.moduleTitle)}</td>
                    <td>${esc(e.score)}/100</td>
                    <td>
                      <span class="tag ${
                        e.result === "Validé" ? "green" :
                        e.result === "Échec" ? "red" : "orange"
                      }">${esc(e.result)}</span>
                    </td>
                    <td>${esc(e.comments || "")}</td>
                  </tr>
                `).join("")
              : '<tr><td colspan="7" class="muted">Aucune évaluation.</td></tr>'
            }
          </tbody>
        </table>
      </div>
    ...
    $("content").innerHTML = `
      <div class="card">
        <h2>Erreur évaluations</h2>
        <p class="muted">${esc(err.code || err.message)}</p>
        <p>Vérifie que les règles Firestore Phase 3.1 sont bien publiées.</p>
      </div>
    `;
  }
}
...
async function openEvaluationForm(){
  ...
  try{
    ...
    const officers = snap.docs
      ...
      .filter(o => o.status !== "Inactif");

    showModal(`
      <h2>Nouvelle évaluation FTO</h2>

      <form id="evalForm">
        <div class="formgrid">

          <label class="field">
            <span>Officier évalué</span>
            <select id="eOfficer" required>
              ${officers.map(o => `
                <option value="${esc(o.uid)}" data-name="${esc(o.name)}">
                  ${esc(o.badge)} — ${esc(o.name)} — ${esc(o.grade)}
                </option>
              `).join("")}
            </select>
          </label>
          ...
          ${criteria.map(c => `
            <label class="criterion">
              ...
              <select class="criterion-score" data-key="${c[0]}">
                ...
                <option value="3" selected>3 — Conforme</option>
        ...
        <label class="field full">
          <span>Commentaires FTO</span>
          <textarea id="eComments" rows="5"
            placeholder="Points forts, erreurs, axes d'amélioration, actions à suivre..."></textarea>
        </label>

        <div class="score-preview">
          Score calculé : <b id="scorePreview">60/100</b>
          — <span id="resultPreview">À revoir</span>
        </div>
    ...
    document.querySelectorAll(".criterion-score").forEach(s => {
      s.addEventListener("change", updateEvalPreview);
    });
    ...
    console.error(err);
    alert("Impossible de charger les officiers. Vérifie les règles Firestore.");
  }
}

function updateEvalPreview(){
  const vals = [...document.querySelectorAll(".criterion-score")]
...
async function saveEvaluation(e){
  ...
  document.querySelectorAll(".criterion-score").forEach(s => {
    values[s.dataset.key] = Number(s.value);
  });
  ...
  const payload = {
    officerId,
    officerName,
    ftoId: window.LSPD.user.uid,
    ftoName: window.LSPD.profile.name,
    moduleCode,
    moduleTitle: module[1],
    criteria: values,
    score,
    result,
    comments: $("eComments").value.trim(),
  ...
  try{
    ...
    await addDoc(collection(db,"audit_logs"), {
      actorId: window.LSPD.user.uid,
      actorName: window.LSPD.profile.name,
      action: "CREATE_EVALUATION",
      targetId: officerId,
      details: `${moduleCode} — ${result} — ${score}/100`,
      createdAt: serverTimestamp()
    });
    ...
    console.error(err);
    $("evalError").textContent =
      "Enregistrement impossible : " + (err.code || err.message);
  }
}
...
async function officers(){
  if(!isCommand()){
    $("content").innerHTML = `
      <div class="card">
        ...
        <p class="muted">Réservé au commandement.</p>
      </div>
    `;
    return;
  }
  ...
  $("content").innerHTML = `
    <div class="card"><p class="muted">Chargement des officiers...</p></div>
  `;
  ...
  try{
    ...
    $("content").innerHTML = `
      <div class="toolbar">
        <input id="officerSearch" class="search"
          placeholder="Rechercher nom, matricule, grade...">

        ${isChief()
          ? '<button class="btn" id="addOfficerBtn">+ Ajouter un profil</button>'
          : ""
        }
      </div>
    ...
    $("officerSearch").addEventListener("input", () => {
      const s = $("officerSearch").value.toLowerCase().trim();

      const filtered = data.filter(o =>
        [o.badge,o.name,o.grade,o.role,o.status]
          .some(v => String(v || "").toLowerCase().includes(s))
      );

      $("officerRows").innerHTML = officerRows(filtered);
    ...
    $("content").innerHTML = `
      <div class="card">
        <h2>Impossible de charger les officiers</h2>
        <p class="muted">${esc(err.code || err.message)}</p>
      </div>
    `;
  }
...
function officerRows(data){
  if(!data.length){
    return `
      <tr>
        <td colspan="6" class="muted">Aucun officier.</td>
      </tr>
    `;
  }
  ...
  return data.map(o => `
    <tr>
      ...
      <td>${esc(o.grade)}</td>
      <td><span class="tag">${esc(o.role)}</span></td>
      <td>${esc(o.status)}</td>

      ${isChief()
        ? `<td>
            <button class="btn secondary edit-officer"
              data-uid="${esc(o.uid)}">
              Modifier
            </button>
          </td>`
...
function bindOfficerEdit(data){
  document.querySelectorAll(".edit-officer").forEach(btn => {
    btn.addEventListener("click", () => {
      const officer = data.find(o => o.uid === btn.dataset.uid);
...
function openOfficerForm(o=null){
  ...
  showModal(`
    <h2>${o ? "Modifier" : "Ajouter"} un profil</h2>

    <p class="muted">
      ${
        o
        ? "Modifie les données Firestore du profil."
        : "Crée d'abord le compte dans Firebase Authentication, puis colle ici son UID."
      }
    </p>
    ...
    <form id="officerForm">
      <div class="formgrid">
        ...
        <label class="field full">
          <span>UID Firebase Authentication</span>
          <input id="fUid"
            ${o ? "readonly" : ""}
            required
            value="${esc(o?.uid || "")}"
        ...
        <label class="field">
          <span>Matricule</span>
          <input id="fBadge" required value="${esc(o?.badge || "")}">
        </label>

        <label class="field">
          <span>Nom RP</span>
          <input id="fName" required value="${esc(o?.name || "")}">
        </label>
        ...
        <label class="field">
          ...
          <select id="fRole">
            ${roles.map(r => `
              <option ${r === o?.role ? "selected" : ""}>${r}</option>
            `).join("")}
          </select>
        </label>
        ...
        <label class="field">
          <span>Division</span>
          <input id="fDivision" value="${esc(o?.division || "Patrol")}">
        </label>
...
async function saveOfficerProfile(e){
  ...
  try{
    ...
    await addDoc(collection(db,"audit_logs"), {
      actorId: window.LSPD.user.uid,
      actorName: window.LSPD.profile.name,
      action: existing.exists() ? "UPDATE_OFFICER" : "CREATE_OFFICER",
      targetId: uid,
      details: `${payload.badge} — ${payload.name} — ${payload.grade}`,
      createdAt: serverTimestamp()
    });
    ...
    <div class="grid module-grid">
      ${scenarios.map(s => `
        <div class="card">
          ...
          <p>${s[2]}</p>
          <p class="muted">${s[3]}</p>
          <button class="btn secondary scenario-btn"
            data-id="${s[0]}">
            Lancer
          </button>
        </div>
...
function startScenario(id){
  ...
  showModal(`
    ...
    <button class="btn" id="closeModal">Terminer</button>
  `);
}
...
async function history(){
  if(!isCommand()){
    $("content").innerHTML = `
      <div class="card"><h2>Accès restreint</h2></div>
    `;
    return;
  }
...
function showModal(html){
  ...
  document.body.insertAdjacentHTML(
    "beforeend",
    `<div class="modal"><div class="modalbox">${html}</div></div>`
  );
