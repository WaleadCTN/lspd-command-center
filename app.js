...
const pages = {
 dashboard:"Dashboard",profile:"Mon profil",manual:"Manuel FTO",modules:"Formations",
...
function applyRoleVisibility(){
  ...
  hide("trainees",!isFTO());
  ["officers","assignments","certifications","records","shifts","requirements","promotions","stats","history","promotionAdvisor"].forEach(p=>hide(p,!isCommand()));
  hide("calendar",!isFTO());
  hide("admin",!isChief());
}
...
function render(page){
  ...
  ({
    dashboard,profile,manual,modules:modulesPage,evaluations,trainees,officers,assignments,
...
async function globalSearch(term){
  ...
  const [usersSnap,evalSnap]=await Promise.all([getDocs(collection(db,"users")),getDocs(collection(db,"evaluations"))]);
  ...
  showModal(`<h2>Recherche globale</h2><h3>Officiers</h3>${users.length?users.slice(0,10).map(o=>`<div class="search-result"><b>${esc(o.badge)} — ${esc(o.name)}</b><span>${esc(o.grade)} • ${esc(o.division||"Patrol")} • ${esc(o.status)}</span></div>`).join(""):'<p class="muted">Aucun officier.</p>'}
  <h3>Évaluations</h3>${evals.length?evals.slice(0,10).map(e=>`<div class="search-result"><b>${esc(e.officerName)} — ${esc(e.moduleCode)}</b><span>${esc(e.result)} • ${esc(e.score)}/100 • FTO ${esc(e.ftoName)}</span></div>`).join(""):'<p class="muted">Aucune évaluation.</p>'}<div class="modal-actions"><button class="btn secondary" id="closeModal">Fermer</button></div>`);
}
